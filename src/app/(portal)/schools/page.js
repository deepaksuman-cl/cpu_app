// File: src/app/(portal)/schools/page.js
...
export default function AllSchoolsPage() {
  ...
  // Object.entries se hum JSON data ko Array me convert kar rahe hain taaki map() chala sakein
  const schoolsList = Object.entries(schoolsData).map(([slug, data]) => ({
    slug,
    ...data
  }));
      ...
      <div className="max-w-7xl mx-auto px-4 lg:px-16 py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          ...
          {schoolsList.map((school, index) => {
            // Extracting data safely (kyunki law aur computer ka json thoda alag hai)
            const title = typeof school.hero?.title === 'object' ? school.hero.title.main : school.hero?.title;
            const bgImage = school.hero?.bgImage || "https://images.unsplash.com/photo-1541339907198-e08756dedf3f?w=800&q=80"; // Fallback image
            const description = school.hero?.description || "Explore our comprehensive programs and world-class faculty.";

            return (
              <div key={index} className="bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 border border-gray-100 group flex flex-col">
